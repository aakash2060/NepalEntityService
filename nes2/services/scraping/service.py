"""Scraping Service implementation for nes2.
...
class ScrapingService:
    ...
    async def normalize_person_data(
        ...
        try:
            ...
            logger.error(f"Error normalizing person data from {source}: {e}", exc_info=True)
            raise
    ...
    async def extract_relationships(
        ...
        try:
            ...
            logger.error(
                f"Error extracting relationships for {entity_id}: {e}",
                exc_info=True
            )
            return []
    ...
    async def translate(
        ...
        try:
            ...
            if target_lang not in ("en", "ne"):
                raise ValueError(f"Invalid target_lang: {target_lang}. Must be 'en' or 'ne'")
